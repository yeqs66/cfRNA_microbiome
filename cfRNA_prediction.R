...
# create model matrix: only selected genes + disease outcome
df_model <- filtered_limma_voom_multiRNA %>%
  dplyr:: select(all_of(selected_genes), disease)

# convert outcome to factor (caret/tidymodels requirement)
...
for (i in 1:nrow(grid)) {
  ...
  for (k in 1:K) {
    ...
    # Store results for plotting later if this is the best model
    all_preds[[k]] <- pred
    all_trues[[k]] <- y_test
  }
  results[[i]] <- list(
    ...
    auc_sd = sd(auc_list),
    # Add all predictions and true values to the results list
...
#########################################################
#if skip train, start here , skip above 
#########################################################
...
# 📊 AUC Drawing for the best parameters
...
# IMPORTANT: scale your data using its own mean & sd
...
## scale using training mean and sd (computed previously)
...
auc_ext <- roc(Y_ext_lyw, pred_ext_lyw)$auc
...
auc_ext <- roc(Y_ext_ncbi, pred_ext_ncbi)$auc
auc_ext
roc_ext <- roc(
  response = Y_ext_ncbi,
...
auc_ext <- roc(Y_ext_combine, pred_ext_combine)$auc
...
# ... (rest of your existing setup code, like 'grid', 'K', 'folds', 'df_model', 'X_all', 'Y_all', 'build_model') ...
####################################################
#can skip if not train from start
####################################################
bmi_results <- list()
...
# 📊 AUC Drawing for the best parameters
# Combine all out-of-fold predictions and true labels
final_trues <- best_model_info$all_trues
final_preds <- best_model_info$all_preds
...
auc_ext <- roc(final_preds, final_trues)$auc
auc_ext
roc_ext <- roc(
...
####################################################
#internal model add bmi + microbiome
####################################################
rm(list = ls())
